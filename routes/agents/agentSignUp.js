//---------------------------------------------signup page call------------------------------------------------------
exports.agentSignUp = function(req, res) {
  message = "";
  if (req.method == "POST") {
    let md5 = require('md5');
    let post = req.body;
    ...
    let sql =
      "INSERT INTO `agentusers`(`first_name`,`last_name`,`mob_num`,`user_name`, `password`) VALUES ('" +
      fname +
      "','" +
      lname +
      "','" +
    ...
    let query = db.query(sql, function(err, result) {
      ...
      res.render("agentSignUp", { message: message });
    });
  } else {
  ...
  if (req.method == "POST") {
    ...
    db.query(sql, function(err, results) {
      if (results.length) {
        ...
        res.render("agentSignIn.ejs", { message: message });
      }
    });
  } else {
...
exports.agentDashboard = function(req, res, next) {
  let user = req.session.user,
    userId = req.session.userId;
  if (userId == null) {
    res.redirect("/agentSignIn");
    return;
  }
  ...
  db.query(sql, function(err, results) {
    res.render("agentDashboard", { user: user });
  });
};
