...
let socket = io();  //create socket connection
let isDriver = true;
let faker = false;  //Initially setting the faker profile as false, if switched to true then it will switch to faker profile and driver icon aminate on Click event
let inited = false;
...
function success(position) {
    if (!inited)
        ...
        let loc = mymarker.getLatLng();
        let latLong = getLatLong(position)
        let angle = setangle(loc.lat, loc.lng, latLong[0], latLong[1])
        mymarker.setIconAngle(angle);
