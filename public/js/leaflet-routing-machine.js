...
polyline.decode = function(str, precision) {
    ...
    // Coordinates have letiable length when encoded, so just keep
	...
	L.Routing.Autocomplete = L.Class.extend({
		...
		_open: function() {
			...
			if (!this._container.parentElement) {
				...
				let scrollX = (window.pageXOffset !== undefined) ? window.pageXOffset
					: (document.documentElement || document.body.parentNode || document.body).scrollLeft;
				let scrollY = (window.pageYOffset !== undefined) ? window.pageYOffset
		...
		_createClickListener: function(r) {
			let resultSelected = this._resultSelected(r);
...
(function() {
	...
	L.Routing.Control = L.Routing.Itinerary.extend({
		...
		onAdd: function(map) {
			let container = L.Routing.Itinerary.prototype.onAdd.call(this, map);
		...
		spliceWaypoints: function() {
			let removed = this._plan.spliceWaypoints.apply(this._plan, arguments);
			return removed;
		},
		...
		_routeSelected: function(e) {
			let route = this._selectedRoute = e.route,
		...
		_updateLines: function(routes) {
			let addWaypoints = this.options.addWaypoints !== undefined ?
		...
		_hookAltEvents: function(l) {
			l.on('linetouched', function(e) {
				let alts = this._routes.slice();
				let selected = alts.splice(e.target._route.routesIndex, 1)[0];
		...
		_updateLineCallback: function(err, routes) {
			if (!err) {
				routes = routes.slice();
				let selected = routes.splice(this._selectedRoute.routesIndex, 1)[0];
				...
				this._pendingRequest = this._router.route(wps, function(err, routes) {
					...
					if (ts === this._requestCount) {
						...
						if (!options.geometryOnly) {
							...
							let selectedRoute = routes.splice(0,1)[0];
...
(function() {
	...
	L.Routing.Formatter = L.Class.extend({
		...
		initialize: function(options) {
			...
			let langs = L.Util.isArray(this.options.language) ?
		...
		formatDistance: function(d /* Number (meters) */, sensitivity) {
			let un = this.options.unitNames || this._localization.localize('units'),
		...
		_round: function(d, sensitivity) {
			let s = sensitivity || this.options.roundingSensitivity,
		...
		_getInstructionTemplate: function(instr, i) {
			let type = instr.type === 'Straight' ? (i === 0 ? 'Head' : 'Continue') : instr.type,
	...
	L.Routing.GeocoderElement = L.Class.extend({
		...
		options: {
			...
			geocoderPlaceholder: function(i, numberWaypoints, geocoderElement) {
				let l = new L.Routing.Localization(geocoderElement.options.language).localize('ui');
				return i === 0 ?
					l.startPlaceholder :
					(i < numberWaypoints - 1 ?
		...
		initialize: function(wp, i, nWps, options) {
			...
			let g = this.options.createGeocoder(i, nWps, this.options),
...
(function() {
	...
	L.Routing.Itinerary = L.Control.extend({
		...
		_toggle: function() {
			let collapsed = L.DomUtil.hasClass(this._container, 'leaflet-routing-container-hide');
			this[collapsed ? 'show' : 'hide']();
		},

		_createAlternative: function(alt, i) {
			let altDiv = L.DomUtil.create('div', 'leaflet-routing-alt ' +
		...
		_createItineraryContainer: function(r) {
			let container = this._itineraryBuilder.createContainer(),
		...
		_onAltClicked: function(e) {
			...
			let j = this._altElements.indexOf(altElem);
			let alts = this._routes.slice();
			let route = alts.splice(j, 1)[0];
...
(function() {
	...
	L.Routing.ItineraryBuilder = L.Class.extend({
		...
		createContainer: function(className) {
			let table = L.DomUtil.create('table', className || ''),
...
(function() {
	...
	L.Routing.Line = L.LayerGroup.extend({
		...
		_findWaypointIndices: function() {
			let wps = this._route.inputWaypoints,
			    indices = [],
			    i;
			for (i = 0; i < wps.length; i++) {
		...
		_findNearestWpBefore: function(i) {
			let wpIndices = this._getWaypointIndices(),
		...
		_onLineTouched: function(e) {
			let afterIndex = this._findNearestWpBefore(this._findClosestRoutePoint(e.latlng));
...
(function() {
	...
	L.Routing.Localization = L.Class.extend({
		initialize: function(langs) {
			...
			for (let i = 0, l = this._langs.length; i < l; i++) {
		...
		localize: function(keys) {
			...
			for (let i = 0, l = this._langs.length; i < l; i++) {
				...
				for (let j = 0, nKeys = keys.length; dict && j < nKeys; j++) {
					key = keys[j];
					value = dict[key];
					dict = value;
				}
...
(function() {
	...
	L.Routing.OSRMv1 = L.Class.extend({
		...
		requiresMoreDetail: function(route, zoom, bounds) {
			...
			let waypoints = route.inputWaypoints,
				i;
			for (i = 0; i < waypoints.length; ++i) {
...
(function() {
	...
	L.Routing.Plan = (L.Layer || L.Class).extend({
		...
		setWaypoints: function(waypoints) {
			let args = [0, this._waypoints.length].concat(waypoints);
		...
		createGeocoders: function() {
			let container = L.DomUtil.create('div', 'leaflet-routing-geocoders ' + this.options.geocodersClassName),
		...
		_createGeocoder: function(i) {
			let geocoder = this.options.createGeocoderElement(this._waypoints[i], i, this._waypoints.length, this.options);
			geocoder
			.on('delete', function() {
		...
		_dragNewWaypoint: function(newWpIndex, initialLatLng) {
			let wp = new L.Routing.Waypoint(initialLatLng),
